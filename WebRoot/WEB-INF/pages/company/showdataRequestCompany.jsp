						...
						<c:forEach items="${companys}" var="current" varStatus="i">
							...
							<tr class="${rowclass}">
								...
								<td nowrap="nowrap" class="tabletd"><c:if
										...
										<span style="color:red">暂时没有项目</span>
									</c:if> 
									<c:set var="isDone" value="0" />
									<c:forEach
									items="${current.relativeProjects}" var="referenced">
											<br/>
											<!--pj修改的内容 :在href里面添加了project_id=${referenced.id} -->
											<a
											href="${pageContext.request.contextPath}/selectCompanyProjects?company_id=${current.id}&page=homePage&project_id=${referenced.id}">${referenced.name}
											</a>
											</c:forEach>
									</td>
								<!-- 项目值的获取============== -->
